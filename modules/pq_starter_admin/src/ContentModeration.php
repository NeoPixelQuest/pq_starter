<?php
namespace Drupal\pq_starter_admin;
...
class ContentModeration {
  ...
  public function transitionTo($state = '') {
    if ($state == '') {
      ...
      \Drupal::logger('pq_starter_admin')->notice($this->message);
      return FALSE;
    }
    ...
    foreach ($allLanguages as $langcode => $languageName) {
      if ($this->entity->hasTranslation($langcode)) {
        \Drupal::logger('pq_starter_admin')->notice(ucfirst($state) . " latest revision $langcode for " . $this->id);
